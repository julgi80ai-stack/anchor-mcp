...
> **v1.18 → v1.19 변경 요약**: **사양이 자기가 도입한 변경을 일곱 판본 동안 따라오지 못한 자리 하나를 고친다**(§7.7). v1.12(§22 3행)가 `created`·`renormalized`를 `changed`에서 **분리해 표시**하도록 정했으나, §7.7의 정본 블록과 회계 불변식 문장은 여섯 버킷인 채 남았다. 사용자 실 저장소로 대조하면 사양이 적은 여섯의 합은 **102**, `requests`는 **208** — **사양이 적어 둔 불변식이 글자 그대로 거짓**이었다. 구현(`Anchor.cache_stats`)은 처음부터 여덟 버킷을 내고 합도 맞으므로 **코드는 바꾸지 않는다** — 판정 기준상 "사양이 스스로 도입한 변경을 못 따라온 경우"다. 전체 목록은 §15 참조.
...
**사양이 자기가 도입한 변경을 따라오지 못했다.** 이 절은 하나를 고친다. v1.12가 `created`·`renormalized`를 `changed`에서 분리해 표시하도록 정하면서(§22 3행) §7.7의 **정본 블록과 불변식 문장을 갱신하지 않았고**, 그 상태로 일곱 판본을 지났다. 회계 불변식은 이 사양이 "이 둘이 깨지면 아래 모든 숫자가 거짓이 된다"고 적어 둔 자리다 — 그 자리가 스스로 거짓인 채였다.
...
**코드는 한 줄도 바꾸지 않았다.** 구현이 옳고 사양이 낡았다 — 판정 기준(CLAUDE.md "SPEC과 코드가 어긋날 때")의 "사양이 스스로 도입한 변경을 못 따라온 경우"에 해당한다.
...
| 22 | **13** | v0.1 완료 기준을 **본문 0바이트**(cache_hit·직접 304)로 실제 개정 — §10 표와 **양쪽** | v1.10 첫 개정이 §10만 고치고 §15에는 §13이라 적어, **사양이 하지 않은 일을 했다고 주장했다**. §13의 기준은 같은 흐름에서 고친 자기 회귀 테스트와 모순인 채 남아 있었다 (D-216) |
