...
**부하 중의 캐시 히트 (v1.9)**: 1행의 목표치는 백그라운드 task의 매칭 루프가 도는 동안에도 성립해야 한다 — Tasks 확장이 존재하는 바로 그 상황이기 때문이다. 매칭 루프는 순수 파이썬이라 GIL 슬라이스를 통째로 쥐고, 캐시 히트 경로는 GIL을 수십 번 얻어야 하므로 대기가 획득 횟수만큼 누적된다(실측 6~14배 초과). 배경 워커의 매칭 루프는 예산 확인 주기마다 GIL을 양보하되, 양보의 잠듦은 두 겹으로 제한된다: **전경 호출이 실제로 진행 중일 때만** 잠들고(굶는 사람이 없는 양보는 순손실 — 잠듦의 참비용인 콜드 캐시 재예열은 상환 불가), **잠든 시간은 앵커 예산에서 청구하지 않는다**(청구하면 같은 앵커의 판정이 동기 경로와 task 경로에서 갈린다 — §6.2의 실효 길이 한계는 호출 경로와 무관해야 한다). 전경과 실제로 겹친 구간에서는 경계 문서의 판정이 `UNRESOLVED`로 보류될 수 있다 — 그것은 실측된 경합의 정직한 보고이며, 재검증으로 회복된다. 벤치마크는 유휴와 부하 중을 **둘 다** 재고(유휴만 재면 이 회귀는 게이트를 통과한다), 매칭 판정 게이트는 전경과 정중 모드 **양쪽**에서 재며 두 결과의 일치를 별도 게이트로 확인한다. 부하 게이트는 부하의 생존(매칭 반복 최소치·스레드 조기 사망)을 스스로 확인한다 — churn이 죽으면 유휴를 재면서 PASS를 찍는다.
...
| 4 | 7.0 | 취소·종료 신호를 문서 사이뿐 아니라 **앵커 사이에서도** 확인한다 — 반응 상한은 배치 크기가 아니라 앵커 하나의 예산 + 진행 중인 네트워크 요청 한 건이다 | 반응 시간이 한 문서의 앵커 수에 비례해 무한정 늘었다(앵커 100개에서 신호 후 13.8초). `tasks/cancel`이 비종결 상태를 반환해 클라이언트가 계속 폴링해야 했다 (D-119). "앵커 하나의 예산"이라고만 적은 초안은 사실이 아니었다 — 네트워크 대기 중에는 신호를 확인하지 않는다 (D-197) |
...
| 8 | **10** | 배경 워커의 양보는 **전경 호출이 진행 중일 때만** 잠들고, 잠든 시간은 **앵커 예산에서 청구하지 않는다**. 매칭 판정 게이트를 전경·정중 모드 양쪽에서 재고 일치를 확인한다 | 양보 비용이 예산 안에서 청구돼 같은 앵커가 동기 경로 MISSING / task 경로 UNRESOLVED로 갈렸다(최악 사례 0→59~60/100, 크레딧만으로는 30/100 — 잠듦의 콜드 캐시 비용은 상환 불가). §7.3이 task를 기본 경로로 정하므로 낮은 한계가 기본이 됐고, 판정 게이트가 전경에서만 돌아 이를 못 봤다 (D-196) |
| 9 | 10 | 부하 중 게이트는 부하의 생존(매칭 반복 최소치·조기 사망)을 스스로 확인한다 | churn 스레드가 임포트 오류로 즉사해도 유휴를 재면서 PASS·exit 0 — 지키려는 회귀가 churn 쪽을 깨는 방식이면 게이트가 스스로 무력화됐다 (D-198) |
