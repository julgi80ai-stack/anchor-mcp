...
> **ttl 정책과 종료 보장 (v1.9)**: `Task.ttl`은 **항상 유한한 실제 보존 기간**이다 — ttl을 생략한 호출에는 서버 기본(30분)을 부여하고, 0·음수는 거부(-32602: 결과 회수가 원천 불가능한 보존 기간)하며, 과대 요청은 상한(24시간)으로 클램프해 실제값을 보고한다. **응답 어디에서도 ttl 키를 생략하지 않는다** — required-nullable 필드는 exclude_none 직렬화에서 키가 사라져 표준 클라이언트의 스키마 검증이 응답 전부를 거부한다. 보존 기간은 프로토콜 정의대로 생성 시점부터 세되, 종결 시점에 실제 보존 기간(경과+요청)으로 갱신해 실행이 오래 걸린 task의 결과가 종결과 동시에 소멸하지 않게 한다. 종료는 **워커가 전부 끝난 뒤에만** 저장소를 해제한다 — 유예를 넘기면 알리고 계속 기다린다(어차피 인터프리터도 워커를 기다린다: 같은 대기를 저장소가 열린 채로 할 뿐이다). 취소·종료 신호는 문서 사이뿐 아니라 **앵커 사이에서도** 확인한다 — 반응 시간의 상한은 배치 크기가 아니라 앵커 하나의 예산이다.
...
| 4 | 7.0 | 취소·종료 신호를 문서 사이뿐 아니라 **앵커 사이에서도** 확인한다 — 반응 상한은 배치 크기가 아니라 앵커 하나의 예산이다 | 반응 시간이 한 문서의 앵커 수에 비례해 무한정 늘었다(앵커 100개에서 신호 후 13.8초). `tasks/cancel`이 비종결 상태를 반환해 클라이언트가 계속 폴링해야 했다 (D-119) |
