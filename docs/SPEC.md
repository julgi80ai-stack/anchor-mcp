...
> **v1.9 → v1.10 변경 요약**: 2차 감사 결함 중 **6단계(회계·설정·CLI, 군집 9~11)** 의 조치를 반영한다. 첫 항목으로, 경합 게이트의 판정을 전경 기준선 대비 UNRESOLVED 증분에서 **크레딧 실지급**으로 교체했다(§10) — 증분 게이트는 유휴 기계에서 통과 불가, 부하 기계에서 무판별이었고, 크레딧 소실 회귀는 기존 어느 게이트에도 걸리지 않았다. 전체 목록은 §15 참조.
>
