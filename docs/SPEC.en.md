...
>
> **v1.1 → v1.2 change summary**: Reflects the results of the license audit. The `trafilatura>=1.8.0` lower bound was made mandatory (§11); MemGator operating guidance was made explicit (§5.2, §9); the provenance policy for test fixtures was split into three grades (§12); and a license gate was added to CI (§12). See §32 for the full list.
>
> **v1.0 → v1.1 change summary**: Reflects the results of the prior-art survey by introducing Memento compatibility (§2, §5.2, §7.8), replacing the anchor matching algorithm with a performance-safe approach (§6.2), expanding the verification states to seven (§6.3), and adopting the Tasks extension of the latest MCP spec (§7.0). See §33 for the full list.
...
**The specification failed to follow a change it introduced itself.** This section fixes one thing. When v1.12 decided that `created` and `renormalized` would be displayed separately from `changed` (§22, row 3), it **did not update §7.7's authoritative block or its invariant sentence**, and that state survived seven revisions. The accounting invariant is the very place where this specification writes "if these two break, every number below them is a lie" — and that place was a lie about itself.
...
**Not one line of code changed.** The implementation was right and the specification was stale — under the project's rule for divergence, this is "the specification failed to follow a change it introduced itself".
